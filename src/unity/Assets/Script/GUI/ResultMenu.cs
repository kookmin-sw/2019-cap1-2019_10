using UnityEngine;
using System;
using System.Collections.Generic;
using System.Linq;

public class ResultMenu : BaseMenu
{
    private bool loading;
    private List<Result> results;
    private Score newestResult;
    //loading = true;

    // Start is called before the first frame update
    void Start()
    {
        backendManager.OnResultLoaded += OnResultLoaded;
    }

    public void OnResult()
    {
        backendManager.GetResult();
    }

    private void OnResultLoaded(List<Result> newResults)
    {
        results = newResults.ToList();

        foreach (Result result in results)
        {
            Debug.Log(result);
        }
        //newestScore = scores.OrderByDescending(s => s.Updated).FirstOrDefault(s => s.Amount == (int)CurrentScore);
        //loading = false;
    }

    //    1. happiness
    //#기쁨 #신남 #흥폭팔 #댄스댄스 #환희 #희열 #스마일 #행복 
    //2. sadness
    //#슬픔 #눈물뚝뚝 #쓸쓸함 #우울 #서러움 #애통 #애처로움 
    //3. surprise
    //#깜짝 #놀람 #화들짝 #멘붕 #당황 #깜놀 #봉변 #혼비백산 
    //4. fear
    //#무서움 #공포 #겁 #두려움 #불안 #섬뜩함 #아찔함 #살벌
    //5. disgust
    //#역겨움 #불쾌 #우웩 #진짜싫음 #메스꺼움 #역함 #넌더리남 
    //6. anger 
    //#화남 #화 #폭발 #짜증 #스트레스 #분노 #역정 #심통 #울화 #성질
    //7. lie
    //#거짓말 #뻥 #다시해보세요 #거짓말쟁이 #거짓 #한번더 #다시 #아니야
}
